# Design-patterns-challenge problems
## Adapter Pattern :
**Problem**: You have a legacy system that provides data in a format incompatible with a new client application. Implement an adapter that converts the legacy data format into a format compatible with the client application without modifying the existing system.
```
...
```
The Adapter Pattern is used to make incompatible interfaces work together. It acts as a bridge between the two interfaces, allowing them to collaborate without changing their existing code.

In the code above the Adapter Pattern is used to adapt data from a legacy system into a format compatible with a new client application. The `adaptLegacyDataToNewFormat()` function serves as the adapter, converting the legacy data format into the new format.
...
```
The Bridge Pattern is a structural design pattern that decouples an abstraction from its implementation so that they can vary independently. It allows the abstraction and implementation to be changed and extended without affecting each other.
In the solution above the Bridge Pattern is used to decouple shapes (abstraction) from rendering modes (implementation) in a drawing application. This allows new shapes and rendering modes to be added independently.
...
In the solution above, the Composite Pattern is used to create a hierarchical structure for managing a company's organizational chart. Each node in the hierarchy represents either an individual employee or a department.
...
## Decorator Pattern:
**Problem:** Extend a basic car rental system to support additional features such as GPS navigation, child seat, and insurance options. Apply the decorator pattern to dynamically add and combine these optional features to a base car rental object without altering its core functionality.
```
